...
sa_colors = ["#ca0020", "#f4a582", "#ffffff", "#92c5de", "#0060b0"]
magmas = plt.get_cmap('magma')
virid = plt.get_cmap('viridis')
plot_colors = magmas(linspace(0, 1, 2048))
...
def plot_1d(model, x_param, fixed_params=None, param_ranges=None, steps=1000):
    """
    Plot a 1d slice from the model.
    Params:
        model: the model to use for predictions.
        x_param: the varying parameter to plot.
        ...
        steps: the number of x values to break the range into..
    """
    ...
    x_vals = linspace(plot_param_ranges[x_param][0], plot_param_ranges[x_param][1], steps)
    ...
    mean, lower, upper = model.predict(data)
    mean, lower, upper = mean.numpy(), lower.numpy(), upper.numpy()

    # Plot:
    f, ax = plt.subplots(1, 1, figsize=(10, 10))
    f.patch.set_facecolor('white')
    ax.plot(x_vals, mean, 'b')
    ax.fill_between(x_vals, lower, upper, alpha=0.5)
    ax.set_title(f"Varying {x_param.lower().replace('_',' ')}", color='black')
    ax.legend(['Predicted Mean', 'Confidence'])
    ax.set_ylabel("GP prediction", color='black')
    ax.set_xlabel(x_param.replace('_',' '), color='black')
    ax.autoscale(enable=True, tight=True)


def plot_2d(model, x_param, y_param, fixed_params=None, param_ranges=None, x_and_y_steps=100, plot_model_uncertainty=False, show_colorbar=True):
    """
    ...
    for i in range(len(mean)):
        ...
        if plot_model_uncertainty:
            val = upper[i] - lower[i]
            plot_data[i % x_and_y_steps][i // x_and_y_steps] = val
            if val > max_uncertainty:
                max_uncertainty = val
    ...
    max_val = 1
    if model.output_type == "Gens to 99 percent":
        max_val = 150
    if plot_model_uncertainty:
        max_val = max_uncertainty
    graph = ax.pcolormesh(plot_data, cmap=plot_cmap, rasterized=True, vmin=0, vmax=max_val)
    ax.set_aspect('equal')
    ...
    ax.set_title(f"{model.output_type}")
    if show_colorbar:
        fig.colorbar(graph, orientation='horizontal', shrink=0.8)
...
def animated_plot(model, x_param, y_param, z_param, fixed_params=None, param_ranges=None, x_and_y_steps=100, z_steps=10):
    ...
    ax.set_aspect('equal')
    fig.suptitle(f"{model.output_type}", y=0.95)
...
def animate(frame_number, ax, model, x_param, y_param, z_param, plot_fixed_params, plot_param_ranges, x_and_y_steps, z_vals):
    ...
    if model.output_type == "Gens to 99 percent":
        max_val = 150
    ax.pcolormesh(plot_data, cmap=plot_cmap, rasterized=True, vmin=0, vmax=max_val)
    ax.set_aspect('equal')
...
def hplot(names, first, first_conf, total, total_conf, ax):
    ...
    ax.barh(y_pos, total, height=0.4, xerr=total_conf, color=sa_colors[3])
    ax.barh(y_pos, first, height=0.4, xerr=first_conf, color=sa_colors[4])
...
def sa_plot(sensitivity_analysis, limit_num_second_order_bars=12):
    ...
    ax.set_title(sa[3])
    b_patch = mpatches.Patch(color=sa_colors[4], label="First order effects")
    lb_patch = mpatches.Patch(color=sa_colors[3], label="Total effects")
